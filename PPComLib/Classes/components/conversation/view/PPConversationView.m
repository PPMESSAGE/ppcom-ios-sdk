...
@implementation PPConversationView
...
- (void)pp_init {
    self.conversationViewMessages = [PPConversationViewMessages new];
    self.conversationViewMessages.translatesAutoresizingMaskIntoConstraints = NO;
    [self addSubview:_conversationViewMessages];
    
    self.inputToolbar = [PPMessageInputToolbar new];
    self.inputToolbar.translatesAutoresizingMaskIntoConstraints = NO;
    [self addSubview:_inputToolbar];
    
    [self configureconversationViewMessagesLayoutConstraints];
    [self configureInputtoolbarLayoutConstraints];
    
}

- (void)configureconversationViewMessagesLayoutConstraints {
    PPPadding(_conversationViewMessages, self, 0.0, PPPaddingMaskLeading | PPPaddingMaskTop | PPPaddingMaskTrailing);
}
...
- (void)configureInputtoolbarLayoutConstraints {
    ...
    // top
    self.inputToolbarMarginTopLayoutConstraint = [NSLayoutConstraint constraintWithItem:_inputToolbar attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:_conversationViewMessages attribute:NSLayoutAttributeBottom multiplier:1.0 constant:.0];

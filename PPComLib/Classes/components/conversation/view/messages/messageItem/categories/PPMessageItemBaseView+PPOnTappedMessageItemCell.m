...
- (void)pp_onTappedMessage:(PPMessage *)message {
    if (self.conversationViewController) {
        [self.conversationViewController endEditing];
    }
...
- (void)pp_onTappedVoiceMessage:(PPMessage *)message
    ...
    PPMessage *messageOnPlaying = [self.conversationViewController pp_messageOnPlayingVoice];
    ...
    [self.conversationViewController reloadTableView];
            ...
            [downloader queryDiskCacheForFileUUID:audioMediaPartNeedToPlay.fileUUID done:^(id obj, NSString *fileUUID, NSString *fileDiskPath) {
                ...
                if (obj) {
                    ...
                    [downloader downloadWithFileUUID:audioMediaPartNeedToPlay.fileUUID withBlock:^(id obj, NSString *fileUUID, NSString *fileDiskPath) {
                        if (!wself || !wself.conversationViewController) {
                            [downloader cancel];
                            return;
                        }
                        
                        // Is there another voice playing ...
                        PPMessage *anotherMessageIsAnimating = [wself.conversationViewController pp_messageOnPlayingVoice];
                        ...
                        if (obj) {
                            ...
                            [wself.conversationViewController reloadTableView];
                        }
...
- (void)didAudioPlayerStopPlay:(AVAudioPlayer*)audioPlayer {
    [self.conversationViewController pp_stopOnPlayingVoice];
    [self.conversationViewController reloadTableView];
}
...
- (void)pp_showImageForMessage:(PPMessage *)message
    ...
    if (self.conversationViewController) {
        [imageViewer showFromViewController:self.conversationViewController transition:JTSImageViewControllerTransition_FromOriginalPosition];
    }

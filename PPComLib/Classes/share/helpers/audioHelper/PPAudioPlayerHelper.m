...
@implementation PPAudioPlayerHelper
...
- (void)setDelegate:(id<PPAudioPlayerHelperDelegate>)delegate {
...
+ (instancetype)shareInstance {
    static PPAudioPlayerHelper *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[PPAudioPlayerHelper alloc] init];
    });
    return instance;
}
...
- (void)sensorStateChange:(NSNotificationCenter *)notification {
    ...
    if ([[UIDevice currentDevice] proximityState] == YES) {
        //黑屏
        PPFastLog(@"Device is close to user");
        [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayAndRecord error:nil];
        
    } else {
        //没黑屏幕
        PPFastLog(@"Device is not close to user");
        [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback error:nil];
        if (!_player || !_player.isPlaying) {
